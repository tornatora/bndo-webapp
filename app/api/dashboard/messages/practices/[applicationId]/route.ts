...
import {
  ensureParticipantCompat,
  ensurePracticeThreadCompat,
  listMessagesCompat,
  maybeCreateAutomaticReply,
  insertMessageCompat,
...
export async function GET(
  ...
  try {
    ...
    const participantMode = await ensureParticipantCompat({
      supabase,
    ...
    const messagesResult = await listMessagesCompat({
      supabase,
      threadId: thread.threadId,
...
export async function POST(
  ...
  try {
    ...
    if (!parsedBody.success) {
      return NextResponse.json({ error: 'Messaggio non valido.' }, { status: 422 });
    }
    ...
    const autoReply = await maybeCreateAutomaticReply({
      supabase,
      threadId: thread.threadId,
      mode: inserted.mode,
