...
async function generateAssistantTextWithOpenAI(args: {
  ...
  const body = {
    ...
    temperature: 0.45,
    max_output_tokens: qaMode ? 250 : 180  // === PATCH: più token per risposte QA ===
  } as const;
...
function isScanReadyAdaptive(profile: UserProfile): ScanAdaptiveReadiness {
  ...
  // === PATCH: Regione + settore/ateco bastano per scansione (anche senza fundingGoal) ===
  const hasTopicSubstitute = Boolean(
    (profile.sector && profile.sector.trim().length >= 3) ||
    (profile.ateco && /\d{2}/.test(profile.ateco))
  );
  const regionPlusTopicOk = Boolean(hasRegion && hasContext && hasTopicSubstitute && !goalText);

  if (!corePilarsOk && !genericWithPrecisionOk && !regionPlusTopicOk) {
      if (!missingSignals.includes('topicPrecision')) {
          missingSignals.push('topicPrecision');
      }
  }

  const isReady = corePilarsOk || regionPlusTopicOk || missingSignals.length === 0;

  return {
    ready: isReady,
    missingSignals: (corePilarsOk || regionPlusTopicOk) ? [] : missingSignals,
